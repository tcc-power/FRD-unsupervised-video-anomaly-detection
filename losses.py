...
def MIL(y_anomaly, y_normal):
    a = int(len(y_anomaly)*0.5)
    b = int(len(y_normal) * 0.5)
    y_anomaly_part = random_choice(y_anomaly,a)
    y_normal_part = random_choice(y_normal, b)
    smooth_loss = torch.sum(y_normal_part) * 0.0008
